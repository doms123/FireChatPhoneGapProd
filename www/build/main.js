...
var UserProvider = (function () {
    function UserProvider(http, afAuth, afdb) {
        ...
        this.afAuth.authState.subscribe(function (user) {
            if (user) {
                _this.loggedUserId = user.uid;
                console.log('this.loggedUserId', _this.loggedUserId);
            }
        });
    }
    UserProvider.prototype.loadUserProfile = function () {
        var _this = this;
        var promise = new Promise(function (resolve, reject) {
            var userRef = __WEBPACK_IMPORTED_MODULE_3_firebase_app__["database"]().ref("/users/" + _this.loggedUserId + "/");
